...
const Lamps = ({ getRenderedValue }) => (
  <div className="material--lamps" data-name="Lamps">
    ...
    />
    <label>CFL</label>
    <input readOnly type="text" name="CFL" value={getRenderedValue('CFL')} />
    ...
    />
    <label>HIDs</label>
    <input readOnly type="text" name="HIDs" value={getRenderedValue('HIDs')} />
    <label>LEDs</label>
    <input
      readOnly
      type="text"
...
const Batteries = ({ getRenderedValue }) => (
...
const Ballast = ({ getRenderedValue }) => (
...
const Specialty = ({ getRenderedValue }) => (
...
const Material = () => {
  ...
  const isMobile = useMemo(() => {
    return windowSize.current[0] < 765;
  }, [windowSize[0]]);
  ...
  const MaterialMap = {
    Lamps: {
      component: <Lamps getRenderedValue={getRenderedValue} />,
      sectionName: 'Lamps',
    },
    Ballast: {
      component: <Ballast getRenderedValue={getRenderedValue} />,
      sectionName: 'Ballast',
    },
    Batteries: {
      component: <Batteries getRenderedValue={getRenderedValue} />,
      sectionName: 'Batteries',
    },

    eWaste: {
      component: <EWaste getRenderedValue={getRenderedValue} />,
      sectionName: 'eWaste',
    },

    Specialty: {
      component: <Specialty getRenderedValue={getRenderedValue} />,
      sectionName: 'Specialty',
    },

    'Haz/Other': {
      component: <Haz getRenderedValue={getRenderedValue} />,
      sectionName: 'Haz/Other',
    },
  };

  const shouldShow = useMemo(() => {
    return (
      Object.keys(MaterialMap).filter((m) => {
        const name = MaterialMap[m].sectionName;
        return materialSections[name].selected;
      }).length > 0
    );
  }, []);
  ...
  if (isMobile)
    ...
    return (
      <section className="section--material" data-name="materials">
        ...
        {Object.keys(MaterialMap).map((m, index) => (
          <React.Fragment key={index}>
            {MaterialMap[m].component}
          </React.Fragment>
        ))}
